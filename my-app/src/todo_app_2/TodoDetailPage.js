...
function TodoDetailPage(props) {
    const {todo} = props;
    const [tdTitle, setTdTitle] = useState(todo.title)
    const [tdContent, setTdContent] = useState(todo.content)
    ...
    function handleInput(e){ //등록버튼 클릭
        ...
        console.log(t,c)
        let obj = {
            no : new Date().getMilliseconds(), //할일구분을 위해 추가필요
    ...
    return (
        <div>
            ...
            <form onSubmit={handleSubmit}>
                <input type='hidden' value={todo.no}></input>
                <label>
            ...
            <button onClick={handleInput}>저장</button>
